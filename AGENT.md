...
| `scripts/export_bench_bundle.sh` | 打包实机日志 tarball |
...
### 12.7 实机 Owner Runbook（Cloud Agent 无法 SSH 时由人工执行）
...
# 2) 快速冒烟 ~5min
bash /data/metaX-inference/scripts/quick_smoke_metax.sh

# 3) 全套 benchmark（含 Phase 1/2/3，多次重启 vLLM）
curl -fsSL https://raw.githubusercontent.com/chenxingqiang/metaX-inference/main/scripts/metax_paste_and_run.sh | bash

# 或仅快速模式
FAST=1 curl -fsSL .../metax_paste_and_run.sh | bash

# 4) 下载结果
bash /data/metaX-inference/scripts/export_bench_bundle.sh
...
| 文件 | 说明 |
|------|------|
| `/data/metax-test-logs/ACCEPTANCE.md` | 自动验收报告 |
| `/data/metax-test-logs/ALL_BENCH_SUMMARY.md` | 全套日志 |
| `/tmp/metax-bench-bundle.tar.gz` | tarball 打包 |

结果可通过 GitHub Issue「实机 Benchmark 结果提交」模板回馈。
